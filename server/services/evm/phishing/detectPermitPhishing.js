...
async function detectPermitPhishing(contractAddress, currencySymbol, chainId) {
    ...
    const abi = await getAbi(contractAddress, currencySymbol);
    let hasPermit = false;
    ...
    if (!hasPermit) {
        const byteCode = await getByteCode(contractAddress, currencySymbol)
    ...
    let unlimitedApprovalRisk = false;
