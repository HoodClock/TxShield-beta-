  ...
  try {
    ...
    if (checkResult.rows.length > 0) {
      ...
      return res
        .status(200)
        .json({
          apiKey: checkResult.rows[0].api_key,
    ...
    const insertQuery =
      "INSERT INTO api_keys (api_key, wallet, tier) VALUES ($1, $2, $3) RETURNING api_key";
    await pool.query(insertQuery, [apiKey, wallet, "free"]);

    return res
      .status(201)
      .json({ apiKey, message: "API key generated successfully" });
  } catch (error) {
...
router.get("/apikey/:wallet", async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "No API key found for this wallet" });
    }
