...
const fakeTokenDetection = async (_tokenAddress) => {
  ...
  const coinList = axios.get(process.env.COIN_GECKO_NAME_SYMBOL_API);

  // fetching coin_id and comparing both symbols
  const coinId = coinList.data.find(coin => coin.symbol.toLowerCase() === tokenInfo.symbol.toLowerCase())?.id;

  // fetching full token data
  const sepecificCoinId = `${process.env.COIN_GECKO_SEPECIFIC_ID.replace('{id}', coinId)}`;
