...
function TransactionDetails({
  ...
  const tokensDelta = useMemo(() => {
   ...
   // Memoize full data array
  const transactionItems = useMemo(
    () => [
      {
        label: "Payload",
        value: "Contract Interaction",
        icon: <FiType className="h-4 w-4 text-blue-500/50" />,
      },
      {
        label: "Value",
        value: requestData?.amount
          ...
          : "0.05 ETH (Proxy)",
        icon: <FiHash className="h-4 w-4 text-blue-500/50" />,
      },
      {
        label: "Profit Detected",
        value: simulateData?.isProfit ? "Yes" : "No",
        icon: <FiTrendingUp className="h-4 w-4 text-blue-500/50" />,
      },
      {
        label: "Target Hash",
        value:
          ...
          "0xC02aa...756Cc2 (WETH)",
        mono: true,
        icon: <FiUserCheck className="h-4 w-4 text-blue-500/50" />,
      },
      {
        ...
        icon: <FiZap className="h-4 w-4 text-blue-500/50" />,
      },
      {
        label: "ETH Delta",
        value: simulateData?.ethDelta || "0",
        icon: <FiTrendingUp className="h-4 w-4 pl-1 text-blue-500/50" />,
      },
      {
        label: "Token Delta",
      ...
      },
      {
        ...
        icon: <FiShield className="h-4 w-4 text-blue-500/50" />,
      },
      {
      ...
      },
      {
        ...
        icon: <FiUser className="h-4 w-4 text-blue-500/50" />,
      },
      {
        label: "Tokens Delta",
        value: tokensDelta,
        ...
        <div className="grid grid-cols-2 gap-3 mt-2">
          {transactionItems.map((item, idx) => (
            <m.div
              ...
              className="group/item p-3 rounded-xl bg-blue-950/20 border border-blue-500/10 flex flex-col"
            >
