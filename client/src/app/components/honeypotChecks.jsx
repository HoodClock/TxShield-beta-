  ...
  return (
    <div className="relative group/honeypot mb-6">
      ...
      <div className="relative z-10 p-4 sm:p-5">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-5 gap-3 pb-4 border-b border-white/10 group-hover/honeypot:border-blue-500/30 transition-colors duration-300">
          <div className="flex items-center">
            <div className={`p-3 rounded-xl border border-${theme.secondary}-500/20 mr-4 ${theme.gradientFrom.replace('from-', 'bg-').replace('/40', '/10')}`}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className={`h-6 w-6 ${theme.iconColor} drop-shadow-[0_0_8px_currentColor]`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
            ...
            <div>
              <h4 className="text-xl font-bold font-mono tracking-widest uppercase text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">Detailed Security Analysis</h4>
              <p className="text-xs font-mono tracking-widest uppercase text-gray-500 mt-1">
                Honeypot Vulnerability Signatures
              </p>
            </div>
          </div>

          <div className="flex gap-3">
            <div className="px-4 py-2 bg-emerald-500/5 border border-emerald-500/20 rounded-lg flex items-center shadow-[inset_0_0_10px_rgba(16,185,129,0.05)]">
              <span className="relative flex h-2 w-2 mr-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <span className="text-xs font-mono font-bold tracking-widest uppercase text-emerald-400 drop-shadow-[0_0_5px_rgba(16,185,129,0.5)]">
                {
            ...
            <div className="px-4 py-2 bg-red-500/5 border border-red-500/20 rounded-lg flex items-center shadow-[inset_0_0_10px_rgba(239,68,68,0.05)]">
              <span className="relative flex h-2 w-2 mr-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
              </span>
              <span className="text-xs font-mono font-bold tracking-widest uppercase text-red-400 drop-shadow-[0_0_5px_rgba(239,68,68,0.5)]">
                {
          ...
          {Object.entries(checks).map(([key, result]) => {
            ...
            return (
              <m.div
                ...
                className={`relative group/check overflow-hidden bg-black/40 border ${colors.border} rounded-xl p-4 transition-all duration-300 cursor-pointer ${colors.bgHover} ${colors.borderHover}`}
              >
                ...
                <div className="flex items-start gap-4">
                  {/* Status Indicator */}
                  <div className="mt-1 relative flex items-center justify-center w-6 h-6 rounded bg-[#0a0a0a] border border-white/10 shadow-[inset_0_0_10px_rgba(0,0,0,0.5)]">
                    {passed ? (
                      <svg className={`w-3.5 h-3.5 ${colors.text} drop-shadow-[0_0_5px_currentColor]`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    ) : (
                      <svg className={`w-3.5 h-3.5 ${colors.text} drop-shadow-[0_0_5px_currentColor]`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                    )}
                  </div>

                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                      <h5 className={`font-mono font-bold tracking-wide uppercase text-sm ${colors.text} group-hover/check:drop-shadow-[0_0_8px_currentColor] transition-all`}>
                        {TITLES[key] || key}
                      </h5>
                      <span className={`text-[10px] px-2 py-0.5 rounded border font-mono uppercase tracking-widest whitespace-nowrap ${colors.tag}`}>
                        {passed ? "CLEAN" : "CRITICAL"}
                      </span>
                    </div>
                    <p className="font-mono text-xs sm:text-sm text-gray-400 group-hover/check:text-gray-300 transition-colors leading-relaxed">
                      {message}
                    </p>
                    ...
                    {resultData?.score && (
                      <div className="mt-4 pt-4 border-t border-white/5">
                        <div className="flex justify-between font-mono text-[10px] text-gray-500 uppercase tracking-widest mb-1.5">
                          <span>Confidence Score</span>
                          <span>{resultData.score} {resultData.totalScore}</span>
                        </div>
                        <div className="w-full bg-[#0a0a0a] border border-white/5 h-1.5 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${colors.dot} shadow-[0_0_8px_currentColor]`}
