...
const chains = [
    ...
    { name: 'SCROLL', icon: <Image src="/Images/logos/scroll.svg" width={16} height={16} alt="Scroll" className="invert opacity-80" /> },
...
export default function ChainTicker() {
    ...
    return (
        <div className="absolute bottom-[72px] sm:bottom-[80px] left-0 w-full overflow-hidden border-t border-white/5 bg-black/40 backdrop-blur-md py-3 z-10 flex">
            {/* Absolute fades on edges to make it look seamless */}
            <div className="absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-[#050505] to-transparent z-20 pointer-events-none"></div>
            <div className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-[#050505] to-transparent z-20 pointer-events-none"></div>
            ...
            >
                {tickerItems.map((chain, index) => (
                    <div key={index} className="flex items-center gap-3 px-6 sm:px-8 shrink-0">
                        <span className="text-gray-600 font-mono text-[10px] tracking-[0.2em] hidden sm:block">PROTECTING</span>
                        <div className="flex items-center gap-2">
                            {chain.icon}
                            <span className="text-white font-mono text-[10px] sm:text-xs tracking-[0.2em] font-bold">{chain.name}</span>
                        </div>
